...
namespace NeuralNetwork
{
    public class MaxPool2D : Layer
    {
        ...
        public override double[,,] GetError(double[,,] error_out, double[,,] diff, double[,,] inarray)
        {
            ...
            double[,,] input = new double[depth, height2, width2];


            double[,,] output = GetOutput(ref inarray);
            ...
            for (int z = 0; z < depth; z++)
            {
                for (int y = 0; y < height2; y++)
                {
                    ...
                    {
                        input[z, y, x] = output[z,y / height,x / width] == inarray[z,y,x] ? error_out[z, y / height, x / width] : 0;//cache_input[z, y, x] == cache_sum[z, y / height, x / width] ? error_out[z, y / height, x / width] * diff[z,y,x] : 0;
                    }
                }
            }
